...
import type { ClientCampaign } from "@/types";
...
export default function ClientCampaignsPage() {
  ...
  const [selected, setSelected] = useState<ClientCampaign | null>(null);

  useEffect(() => {
    api.get("/client-campaigns")
      .then((res) => {
        const list: ClientCampaign[] = res.data;
        setCampaigns(list);
            ...
            <div className="ds-card" style={{ overflow: "hidden" }}>
              {filtered.length === 0 ? (
                <div style={{ padding: "48px 20px", textAlign: "center", color: "var(--ink-3)", fontSize: 13 }}>
                  暂无活动 ·{" "}
                  <Link href="/client-campaigns/new" style={{ color: "var(--da-accent)", fontWeight: 600 }}>新建活动</Link>
                </div>
              ) : (
                filtered.map((c, idx) => {
              ...
              <div className="ds-col" style={{ gap: 16 }}>
                <div className="ds-card">
                  <div className="ds-card-section ds-between">
                    ...
                    <div className="ds-row" style={{ gap: 6 }}>
                      <Link href={`/client-campaigns/${selected.id}`} className="ds-btn ds-btn-outline ds-btn-sm">
                ...
                <div className="ds-card">
                  <div className="ds-card-section ds-between">
                    ...
                    <Link href={`/client-campaigns/${selected.id}`} className="ds-btn ds-btn-ghost ds-btn-sm">
                      编辑步骤 <ArrowRight className="h-[13px] w-[13px]" />
                    </Link>
                  </div>
                  {(selected.steps ?? []).length === 0 ? (
                    <div style={{ padding: "24px 20px", textAlign: "center", color: "var(--ink-3)", fontSize: 13 }}>
                      暂无步骤，<Link href={`/client-campaigns/${selected.id}`} style={{ color: "var(--da-accent)", fontWeight: 600 }}>前往配置</Link>
                    </div>
                  ) : (
