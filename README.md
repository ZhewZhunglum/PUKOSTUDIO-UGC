...
当前版本支持达人管理、CSV/Excel 导入导出、邮件模板、多步外联序列（自动跟进）、A/B 主题测试、发送时间窗口、抑制名单与退订、邮箱账号验证与预热爬坡、邮件打开追踪、实时发送进度、数据看板，以及带 AI 降级策略的收件箱工作流。
...
周期任务依赖 Celery beat：多步序列的自动跟进（每 15 分钟扫描）、发信账号预热升档与每日计数清零都由 beat 驱动，开发和生产都应保持运行：
...
活动支持 1-5 步邮件序列：首封立即发送，跟进步骤按设定延迟自动发给未回复的达人（回复/退信/退订即停止）。可选配置 A/B 主题测试（50/50 确定性分流，详情页对比打开率）和发送时间窗口（按收件人时区限定发送时段）。

## CSV / Excel Import

达人导入支持 CSV 和 Excel（`.xlsx`）。推荐表头：
...
- 导入失败：确认上传的是 `.csv` 或 `.xlsx`，并且至少包含 `name` 列。
